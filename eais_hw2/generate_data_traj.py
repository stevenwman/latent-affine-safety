...
def gen_one_traj_img(x_min, x_max, y_min, y_max, u_max, radius, dt, v, dpi, rand=-1):
	...
	# pi wrapping
	if states[2] < 0: states[2] += 2*np.pi
	if states[2] > 2*np.pi: states[2] -= 2*np.pi
	state_obs, img_obs, state_gt, dones, acs = ([] for _ in range(5)) # forgive me
	...
	for t in range(100):
		...
		# ken said it doesn't matter 
		if t == 99: dones.append(1)
		else: dones.append(1)
		acs.append(ac)
...
def generate_trajs(x_min, x_max, y_min, y_max, u_max, r, dt, v, num_pts, dpi):
	...
	with open('wm_demos_cont_act'+str(dpi)+'.pkl', 'wb') as f:
		pickle.dump(demos, f)
