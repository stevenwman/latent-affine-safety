...
from .DDQNAvoid import DDQNAvoid, Transition


class DDQNSingleAvoid(DDQNAvoid):
  """
  ...
  def __init__(
    ...
    """
    super(DDQNSingleAvoid, self).__init__(CONFIG)
  ...
  def update(self, addBias=False):
    ...
    (non_final_mask, non_final_state_nxt, state, action, reward, g_x) = self.unpack_batch(batch)
    ...
    # == get a' by Q_network: a' = argmax_a' Q_network(s', a') ==
    with torch.no_grad():
      self.Q_network.eval()
      action_nxt = (
    ...
    if self.mode == "RA" or self.mode =='avoid':
  ...
  def learn(
    ...
    Args:
        ...
        warmupBuffer (bool, optional): fill the replay buffer if True.
            Defaults to True.
        warmupQ (bool, optional): train the Q-network by g_x if
            True. Defaults to False.
        warmupIter (int, optional): the number of iterations in the
