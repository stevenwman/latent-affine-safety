...
def qp_filter(nominal_action, vfunc, vgrad, state, u_max, eps, dubins, dt):
    A = compute_jacobian(lambda state_in: dubins.discrete_rk4(state_in, torch.zeros((1,1)), dt), state)
    B = compute_jacobian(lambda action: dubins.discrete_rk4(state, action, dt), torch.zeros((1,1)))
    ...
    l = np.array([eps - vfunc(dubins.discrete_rk4(state, torch.zeros((1, 1)), dt).squeeze()), -u_max])
...
def sampling_filter(nominal_action, vfunc, vgrad, state, u_max, eps, dubins, dt, num_samples):
    ...
    if safety_V <= eps:
        ...
        next_states = dubins.discrete_rk4(state.repeat(num_samples, 1), possible_actions, dt)

        vs = []
        for i in range(len(possible_actions)):
            if vfunc(next_states[i, :]) > 0.03:
